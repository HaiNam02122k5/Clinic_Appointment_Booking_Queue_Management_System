...
namespace Clinic.Application.UnitTests.Features.WorkSchedules.Queries
{
    public class GetRequestedShiftsTests
    {
        ...
        public async Task TestGetRequestedShifts()
        {
            ...
            var query = new GetDoctorRequestedShiftsQuery(doctor.Id, DateOnly.FromDateTime(now), DateOnly.FromDateTime(now.AddDays(15)));
            var result = await handler.Handle(query, CancellationToken.None);
            Assert.Equal(2, result.Schedules.Count());

            var query2 = new GetDoctorRequestedShiftsQuery(doctor.Id, DateOnly.FromDateTime(now.AddDays(2)), DateOnly.FromDateTime(now.AddDays(15)));
            var result2 = await handler.Handle(query2, CancellationToken.None);
            Assert.Single(result2.Schedules);

            var query3 = new GetDoctorRequestedShiftsQuery(doctor.Id, DateOnly.FromDateTime(now.AddDays(11)), DateOnly.FromDateTime(now.AddDays(15)));
            var result3 = await handler.Handle(query3, CancellationToken.None);
            Assert.Empty(result3.Schedules);

            var query4 = new GetDoctorRequestedShiftsQuery(doctor.Id, DateOnly.FromDateTime(now.AddDays(-1)), DateOnly.FromDateTime(now.AddDays(40)));
        ...
        public async Task TestGetRequestedShiftsForNonExistingDoctor()
        {
            ...
            var handler = new GetDoctorRequestedShiftsHandler(doctorRepository, workScheduleRepository);
            var now = DateTime.UtcNow;
            var query = new GetDoctorRequestedShiftsQuery(Guid.NewGuid(), DateOnly.FromDateTime(now.AddMinutes(-1)), DateOnly.FromDateTime(now.AddDays(15)));
