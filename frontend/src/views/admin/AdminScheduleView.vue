  ...
  <div class="space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-xs sm:text-sm text-slate-500">
          Quản lý ca trực, phân công lịch khám và xem phân bổ theo tuần
        </p>
      </div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-2xs">
      <div class="flex items-center gap-2">
        <button
          type="button"
          class="rounded-xl border border-slate-200 px-3.5 py-2 text-xs sm:text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-colors"
          @click="previousWeek"
        >
          ← Tuần trước
        </button>
        <button
          type="button"
          class="rounded-xl border border-slate-200 px-3 py-2 text-xs sm:text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors"
      ...
      <div class="text-center">
        ...
        <div class="mt-0.5 text-sm sm:text-base font-bold text-[#0E4D92]">
          {{ selectedWeekLabel }}
        </div>
      </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xs">
      <div class="flex items-center justify-between border-b border-slate-200 p-5 bg-slate-50/50">
        <div>
          <h2 class="text-sm font-bold text-slate-800 uppercase tracking-wider">
            Bảng phân ca làm việc chi tiết
          </h2>
            ...
            >
              ...
              <td class="px-5 py-4">
                ...
                <div class="text-xs text-slate-500 mt-0.5 font-medium">
                  {{ row.doctor.specialty || 'Chưa phân khoa' }}
                </div>
              </td>
              ...
              >
                ...
                <!-- Add shift quick button on empty slot -->
                <button
                  v-else
                  type="button"
                  class="group flex h-14 w-full items-center justify-center rounded-lg border border-dashed border-slate-200 text-slate-300 hover:border-violet-300 hover:bg-violet-50/40 hover:text-violet-600 transition-all cursor-pointer"
                  title="Thêm ca cho ngày này"
                  @click="openCreateModal(row.doctor.id, dayItem.date)"
                >
                  <span class="text-xs font-semibold opacity-0 group-hover:opacity-100">+ Ca</span>
                  <span class="text-slate-300 group-hover:hidden">—</span>
                </button>
              </td>
              ...
              <td class="px-4 py-4 text-right">
                <button
                  type="button"
                  class="inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-bold text-[#0E4D92] hover:bg-blue-50 border border-blue-100 transition-colors"
                  @click="openDoctorDetail(row.doctor)"
                >
                  👁️ Xem
                </button>
